...
func New(buf []byte) (*LocationConfig, error) {
	loc := &LocationConfig{}
	if err := yaml.Unmarshal(buf, loc); err != nil {
		return nil, err
	}
	if loc.IsLocationEmpty() {
		loc.Latitude = defaultLatitude
		loc.Longitude = defaultLongitude
		loc.LocTimestamp = time.Time{}
		loc.Altitude = defaultAltitude
